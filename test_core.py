...
def test_format_method_with_fmt(setup_eq):
    ...
    spec = {'ident': 'dataInicial', 'size': 8, 'fmt': lambda d : format(d, '%d%m%Y')}
...
def test_format_method_with_fmt(setup_eq):
    ...
    spec = {'ident': 'dataInicial', 'size': 2, 'fmt': lambda d : format(d, '%d%m%Y')}
    with pytest.raises(Exception):
        assert setup_eq.fmt(spec, reg) == datetime.now().strftime('%d%m%Y')
